...
const crashed = await request(`/api/v1/checkout/${checkoutId}/confirm`, {
  method: 'POST',
  token,
  body: { paymentRef: `pay_race_${Date.now()}` },
});

if (crashed.status < 500) {
  fail(`Expected a 500 from the checkout race, got ${crashed.status}.`, crashed);
}

console.log('Reproduced production checkout race');
console.log(`HTTP ${crashed.status}: ${crashed.data.error || 'Internal server error'}`);
console.log('Bug: confirmCheckout does not await lockAndReserveStock, which sets context.items / paymentRef to undefined before createPaidOrder reads items[0].product.');
console.log('Stack files: checkout.controller.js → checkout.service.js → order.service.js → checkout.inventory.js');
await new Promise((resolve) => setTimeout(resolve, 1200));
console.log('Open Pulse /errors — the agent should trace those four files.');
